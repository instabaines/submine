...
class gSpan(object):
    ...
    def __init__(self,
        ...
        # NEW: container for structured results
        # each entry will be a dict with graph, support, etc.
        self.results = []
    ...
    def _report(self, projected):
        ...
        # keep the pandas table as you already do
        ...
        # NEW: store a structured record for this pattern
        where_graph_ids = list(set([p.gid for p in projected])) if self._where else None
        self.results.append(
            {
                "dfs_code": copy.copy(self._DFScode),
                "graph": g,
                "support": self._support,
                "num_vertices": self._DFScode.get_num_vertices(),
                "where": where_graph_ids,
                "description": display_str,
            }
        )
        ...
        if self._visualize:
            g.plot()
        if self._where:
            print('where: {}'.format(where_graph_ids))
        print('\n-----------------\n')
